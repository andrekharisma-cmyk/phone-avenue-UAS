...
interface HeaderProps {
  cartCount: number;
  onSearch: (query: string) => void; // Fungsi untuk mengirim kata kunci
  activeFilter?: string;
...
export default function Header({ 
  ...
  // Fungsi yang dijalankan saat mengetik
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    setSearchQuery(value);
    onSearch(value); // Langsung kirim ke page.tsx tanpa perlu klik tombol
  };
  ...
  return (
    <header className="sticky top-0 z-40 bg-white shadow-sm">
      ...
      <div className="px-8 py-6 flex items-center justify-between bg-white">
        ...
        {/* Search Bar yang Fungsional */}
        <div className="flex-1 max-w-2xl mx-12 relative">
          <input 
            type="text" 
            value={searchQuery}
            onChange={handleChange} // Deteksi ketikan
        ...
        <div className="flex items-center space-x-8">
          {/* ... (bagian profil tetap sama seperti sebelumnya) ... */}
          ...
          <Link href="/pembayaran" className="relative group p-2">
